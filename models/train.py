        ...
        for i in range(0, len(training_data), MINIBATCH_SIZE):
            ...
            def disc_cost(d_weights):
                ...
                for j in range(MINIBATCH_SIZE):
                    D_real = prob_real(real_disc_circuit(d_weights, x=data[j][0] + [data[j][1]]))
                    G_real = gen_output(real_gen_circuit(gen_weights, x=data[j][0]))
                    D_fake = prob_real(real_disc_circuit(d_weights, x=data[j][0] + [G_real]))
            ...
            def gen_cost(g_weights):
                cost = 0.0
                for j in range(MINIBATCH_SIZE):
                    G_real = gen_output(real_gen_circuit(g_weights, x=data[j][0]))
                    D_fake = prob_real(real_disc_circuit(disc_weights, x=data[j][0] + [G_real]))
                    cost -= np.log(D_fake)
                cost /= MINIBATCH_SIZE
                return cost
